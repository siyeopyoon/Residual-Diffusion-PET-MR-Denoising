...
from training import training_loop_multigpu_unet
...
use_cuda=True
keyfolder="PatchDiffusion_unet"

data_dir ="/external/2_Datasets/12_PET_MR7T/dataset_paired/training/"
results_dir = f"/external/1_Codes/4_ScBridge/PatchDiffusion_unet/results_unet/"
...
#        c = dnnlib.EasyDict()
    ...
    c.dim3d=opts.dim3d
    c.network_kwargs.update(model_type='UNet3D', embedding_type='positional', encoder_type='standard',
                            decoder_type='standard')
    c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1, 1, 1], model_channels=64, channel_mult=[2, 2, 2])
    c.network_kwargs.class_name = 'training.networks.Patch_3D'
    c.loss_kwargs.class_name = 'training.patch_loss.Patch_Loss_3D_Conditional'
...
#    dist.print0(f'Class-conditional:       {c.dataset_kwargs.use_labels}')
   ...
   # else:
    #    c.device = torch.device('cpu')
    # Train.
    training_loop_multigpu_unet.training_loop(**c)
