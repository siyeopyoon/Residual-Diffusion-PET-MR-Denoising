...
rootpath = "/external"
itersname="000460"
network_dir=f"{rootpath}/1_Codes/PatchDiffusion_z_conditioning/results/network-snapshot-{itersname}.pkl"
out_dir = f"{rootpath}/1_Codes/PatchDiffusion_z_conditioning/results/"
data_dir = f"{rootpath}/2_Datasets/3_WristCTs/trimmedPairs/Test/"
...
def main(network_pkl, resolution, on_latents, embed_fq, mask_pos, x_start, y_start, image_size, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.CustomDataset_Test', path=data_dir,
    ...
    for batch_seeds in range(len(dataset_iterator)):
        ...
        volumes, APs, LATs, positions = [], [], [], []
        for _ in range(batch_size):
            volume_, AP_, LAT_, position_ = next(dataset_iterator)
        ...
        images = torch.cat([APsimages,LATsimages], dim=1)
        positions = torch.cat(positions, dim=0)

        del AP_,LAT_, volume_, position_,APsimages,LATsimages
        ...
        d=160
        image_channel=1
        latents = torch.randn([batch_size, image_channel, image_size, image_size, d], device=device)

        # Generate images.
        sampler_kwargs = {key: value for key, value in sampler_kwargs.items() if value is not None}
        images = edm_sampler(net, latents, images, positions, mask_pos=False, class_labels=None, randn_like=torch.randn_like, **sampler_kwargs)
        ...
        np.save(f'{out_dir}_network{itersname}_{batch_seeds}_50step_generated.npy',images_np)

        #volumes = torch.squeeze(latents)
        volumes = torch.squeeze(volumes)
        volumes_np = volumes.cpu().numpy()
        np.save(f'{out_dir}_network{itersname}_{batch_seeds}_50step_volume.npy',volumes_np)
    # Done.
    dist.print0('Done.')
