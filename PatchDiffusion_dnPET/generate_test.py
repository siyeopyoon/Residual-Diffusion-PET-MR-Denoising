...
itersnames = ["001638"]
network_dir=f"{rootpath}/1_Codes/4_ScBridge/PatchDiffusion_dnPET/results/network-snapshot-{itersnames[0]}.pkl"
out_dir = f"{rootpath}/Random/12_PET_MR7T/{itersnames[0]}/"
...
def main(network_pkl, resolution, on_latents, embed_fq, mask_pos, x_start, y_start, image_size, outdir, subdirs,
    ...
    for itersname in itersnames:
        network_pkl =f"{rootpath}/1_Codes/4_ScBridge/PatchDiffusion_dnPET/results/network-snapshot-{itersname}.pkl"
        # Load network.
        ...
        for batch_seeds in range(len(dataset_iterator)):
            ...
            images = torch.cat([mr, less], dim=1)
            ...
            for a_index in range(averaging):
                ...
                output= edm_sampler(net, latents, images, pos, mask_pos=False, class_labels=None,
            ...
            averaged=averaged*rescaler+offset
            est_path = f'{out_dir}{str(name)}_{str(dose)}_generated_network{itersname}.npy'
            np.save(est_path, averaged)


            less_path = f'{out_dir}{str(name)}_{str(dose)}_less.npy'

            if not os.path.exists(less_path):
                less = torch.squeeze(less)
                less = less.cpu().numpy()

                less = less * rescaler + offset
                np.save(less_path, less)


            full_path=f'{out_dir}{str(name)}_full_original.npy'
            if not os.path.exists(full_path):
                full_np = torch.squeeze(full)
                full_np = full_np.cpu().numpy()

                full_np = full_np * rescaler + offset

                np.save(full_path, full_np)
