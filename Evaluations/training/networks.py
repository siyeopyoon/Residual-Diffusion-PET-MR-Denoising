...
@persistence.persistent_class
class NoPatch_2D_noMR(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        out_channels    = None,
    ...
    def forward(self,src, force_fp32=False, **model_kwargs):


        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and src.device.type == 'cuda') else torch.float32

        x_in = torch.cat([src], dim=1)
        F_x = self.model((x_in).to(dtype), **model_kwargs)
        assert F_x.dtype == dtype
...
class DnCNN(nn.Module):
    def __init__(self,
                 ...
                 depth=17, use_bnorm=True, kernel_size=3):
        ...
        layers.append(nn.Conv2d(in_channels=in_channels, out_channels=model_channels, kernel_size=kernel_size, padding=padding, bias=True))
        layers.append(nn.ReLU(inplace=True))
        for _ in range(depth-2):
            layers.append(nn.Conv2d(in_channels=model_channels, out_channels=model_channels, kernel_size=kernel_size, padding=padding, bias=False))
            layers.append(nn.BatchNorm2d(model_channels, eps=0.0001, momentum = 0.95))
            layers.append(nn.ReLU(inplace=True))
        layers.append(nn.Conv2d(in_channels=model_channels, out_channels=out_channels, kernel_size=kernel_size, padding=padding, bias=False))
...
class SimpleUnet(nn.Module):
    ...
    def create_model(self):
        ...
        # block_1_1
        block_1_1 = []
        block_1_1.extend(self.add_block_conv(in_channels=3, out_channels=64, kernel_size=kernel_size, stride=1,
    ...
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f'{res}x{res}_down'] = UNetBlock_natural(in_channels=cout, out_channels=cout, down=True,
                                                          **block_kwargs)
                if encoder_type == 'skip':
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = (res in attn_resolutions)
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock_natural(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
        skips = [block.out_channels for name, block in self.enc.items() if 'aux' not in name]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock_natural(in_channels=cout, out_channels=cout, attention=True,
                                                         **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock_natural(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock_natural(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res in attn_resolutions)
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock_natural(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
