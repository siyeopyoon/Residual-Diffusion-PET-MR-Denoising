...
class CustomDataset_PET_Denoise(torch.utils.data.Dataset):
    def __init__(self,
                 path,  # Path to directory or zip.
                 patchsize=128,):
        mr_path = path + "mr/"
        ...
        for file_idx in filenames:
            ...
            for pet_noisy_item in pet_noisy:
                ...
                self.dataset.append({"mr": zpad_mr, "full_dose": zpad_full_dose, "less_dose": zpad_less_dose})
    ...
    def patchfy(self, mr,full_dose,less_dose):

        x_start = np.random.randint(0, 160 - self.patchsize)
        y_start = np.random.randint(0, 160 - self.patchsize)
        z_start = np.random.randint(0, 160 - self.patchsize)

        mr_ext = mr[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]
        full_dose_ext= full_dose[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]
        less_dose_ext= less_dose[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]


        position_x = self.x_pos[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]
        position_y = self.y_pos[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]
        position_z = self.z_pos[:,x_start:x_start + self.patchsize,y_start:y_start + self.patchsize,z_start:z_start + self.patchsize]

        position = np.concatenate([position_x,position_y,position_z],axis=0)


        return mr_ext, full_dose_ext, less_dose_ext, position
    ...
    def __getitem__(self, index: int):
        ...
        return self.patchfy(mr,full_dose,less_dose)


    def __len__(self) -> int:
        return len(self.dataset)

class CustomDataset_PET_Denoise_2D(torch.utils.data.Dataset):
    def __init__(self,
                 path):
        mr_path = path + "mr/"
        full_dose_path = path + "imgfull/"
        ...
        for file_idx in filenames:
            ...
            for pet_noisy_item in pet_noisy:
                ...
                for z_idx in range (10,zpad_mr.shape[-1]-10):
                    ...
                    self.dataset.append({"mr": zpad_mr[:,:,z_idx], "full_dose": zpad_full_dose[:,:,z_idx], "less_dose": zpad_less_dose[:,:,z_idx]})
    ...
    def __getitem__(self, index: int):
        mr,full_dose,less_dose=self.normalize_residual(self.dataset[index]["mr"],
                            self.dataset[index]["full_dose"],
                            self.dataset[index]["less_dose"])
        return mr,full_dose,less_dose


    def __len__(self) -> int:
        return len(self.dataset)

class CustomDataset_PET_Denoise_test(torch.utils.data.Dataset):
    def __init__(self,
                 path):
        mr_path = path + "mr/"
        full_dose_path = path + "imgfull/"
    ...
    def normalize_residual(self,mr,full_dose,less_dose):
        ...
        return nrm_mr,nrm_full_dose,nrm_less_dose,l_min, l_max


    def patchfy(self, mr,full_dose,less_dose,l_min, l_max ,name,dose):
        position = np.concatenate([self.x_pos, self.y_pos, self.z_pos],axis=0)

        return mr, full_dose, less_dose, position,l_min, l_max ,name,dose



    def __getitem__(self, index: int):
        mr,full_dose,less_dose,l_min, l_max =self.normalize_residual(self.dataset[index]["mr"],
                            self.dataset[index]["full_dose"],
                            self.dataset[index]["less_dose"])
        return self.patchfy(mr,full_dose,less_dose,l_min, l_max,
...
class CustomDataset_PET_Denoise_test_selective(torch.utils.data.Dataset):
    def __init__(self,
                 path,
                 selective):
        mr_path = path + "mr/"
        ...
        pet_noisy=selective # 'img10ds' is not seen.
