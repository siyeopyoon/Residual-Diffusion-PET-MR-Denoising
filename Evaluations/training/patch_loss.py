...
class Patch_EDMLoss:
    ...
    def pachify3D(self, images, patch_size, padding=None):
        device = images.device
        batch_size, resolution = images.size(0), images.size(2)

        if padding is not None:
            padded = torch.zeros((images.size(0), images.size(1), images.size(2) + padding * 2,
                                  images.size(3) + padding * 2, images.size(4) + padding * 2), dtype=images.dtype, device=device)
            padded[:, :, padding:-padding, padding:-padding, padding:-padding] = images
        else:
            padded = images

        h, w, d = padded.size(2), padded.size(3), padded.size(4)
        ...
        x_pos = torch.arange(tw, dtype=torch.long, device=device).unsqueeze(0).repeat(th, 1, 1).unsqueeze(0).unsqueeze(0).repeat(batch_size, 1, 1, 1, 1)
        y_pos = torch.arange(th, dtype=torch.long, device=device).unsqueeze(1).repeat(1, tw, 1).unsqueeze(0).unsqueeze(0).repeat(batch_size, 1, 1, 1, 1)
        z_pos = torch.arange(th, dtype=torch.long, device=device).unsqueeze(2).repeat(1, 1, td).unsqueeze(0).unsqueeze(0).repeat(batch_size, 1, 1, 1, 1)
        x_pos = x_pos + j.view(-1, 1, 1, 1, 1)
        y_pos = y_pos + i.view(-1, 1, 1, 1, 1)
        z_pos = z_pos + i.view(-1, 1, 1, 1, 1)

        x_pos = (x_pos / (resolution - 1) - 0.5) * 2.
        y_pos = (y_pos / (resolution - 1) - 0.5) * 2.
        z_pos = (z_pos / (resolution - 1) - 0.5) * 2.
        images_pos = torch.cat((x_pos, y_pos, z_pos), dim=1)

        return padded, images_pos


    def __call__(self, net, images, patch_size, resolution, labels=None, augment_pipe=None,dim3d=3):
        if dim3d==3:
            images, images_pos = self.pachify3D(images, patch_size)
...
@persistence.persistent_class
class Patch_EDMLoss_3D_Conditional:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data
    ...
    def __call__(self, net, volumes, images, images_pos, resolution, labels=None, augment_pipe=None,dim3d=3):

        rnd_normal = torch.randn([volumes.shape[0], 1, 1, 1, 1], device=volumes.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data) ** 2

        y, augment_labels = augment_pipe(volumes) if augment_pipe is not None else (volumes, None)
        n = torch.randn_like(y) * sigma
        yn = y + n

        D_yn = net(yn, sigma, imagecondition=images, x_pos=images_pos, class_labels=labels,
...
@persistence.persistent_class
class Patch_Loss_3D_Conditional:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data
    ...
    def __call__(self, net, trg, src, images, images_pos, resolution, labels=None, augment_pipe=None,dim3d=3):

        D_yn = net(src=src,imagecondition=images, x_pos=images_pos)
...
@persistence.persistent_class
class NoPatch_Loss_2D_Conditional:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data
    ...
    def __call__(self, net, trg, src, images, resolution, labels=None, augment_pipe=None,dim3d=3):

        D_yn = net(src=src,imagecondition=images)

        loss = torch.abs(D_yn - trg)
        return loss
...
@persistence.persistent_class
class NoPatch_Loss_2D_Conditional_noMR:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data
    ...
    def __call__(self, net, trg, src, resolution, labels=None, augment_pipe=None,dim3d=3):
