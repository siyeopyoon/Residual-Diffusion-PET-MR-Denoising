...
steps=100
repeats = 50
network_dir=f"{rootpath}/Random/MICCAI_2024/petmr/Models/diffusion/network-snapshot-{itersnames}.pkl"
...
outroot_infer = f"{rootpath}/Random/MICCAI_2024/petmr/Uncertainty/diffusion_{itersnames}_{steps}/"
...
def main(network_pkl, resolution, on_latents, embed_fq, mask_pos, x_start, y_start, image_size, outdir, subdirs,
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.CustomDataset_PET_Denoise_test_selective', path=data_dir, selective= "img10ds")
    ...
    for batch_seeds in range(len(dataset_iterator)):
        ...
        for rid in range (repeats):
            ...
            latents = torch.randn([batch_size, image_channel, images.shape[-3], images.shape[-2], images.shape[-1]],
                                  device=device)
            ...
            recon_path = os.path.join(outroot_infer+ dose, name+ "_{repeats}.nii.gz")
