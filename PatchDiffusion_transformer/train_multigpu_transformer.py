...
from training import training_loop_multigpu_transformer
...
data_dir ="/external/2_Datasets/12_PET_MR7T/dataset_paired/training/"
results_dir = f"/external/1_Codes/4_ScBridge/PatchDiffusion_transformer/results_transformer/"

@click.command()

#SY 3D options
@click.option('--dim3d',      help='Dimensionality of in-outs', metavar='INT',           type=click.IntRange(min=1), default=2, show_default=True)
...
@click.option('--batch',         help='Total batch size', metavar='INT',                            type=click.IntRange(min=1), default=16, show_default=True)
...
def main(**kwargs):
    ...
    c = dnnlib.EasyDict()
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.CustomDataset_PET_Denoise',
                                       path=opts.data,
...
#        c = dnnlib.EasyDict()
        c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.CustomDataset_PET_Denoise_2D',
                                           path=opts.data)
    ...
    c.dim3d=opts.dim3d
    c.network_kwargs.update(model_type='restormer2D', embedding_type='positional', encoder_type='standard',
                            decoder_type='standard')
    c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1, 1, 1], model_channels=64, channel_mult=[2, 2, 2])
    c.network_kwargs.class_name = 'training.networks.NoPatch_2D'
    c.loss_kwargs.class_name = 'training.patch_loss.NoPatch_Loss_2D_Conditional'
...
#    dist.print0(f'Class-conditional:       {c.dataset_kwargs.use_labels}')
   ...
   # else:
    #    c.device = torch.device('cpu')
    # Train.
    training_loop_multigpu_transformer.training_loop(**c)
